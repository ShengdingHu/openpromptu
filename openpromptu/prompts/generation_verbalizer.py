...
class GenerationVerbalizer(Verbalizer):
    ...
    def wrap_one_example(self, example: Optional[InputExample]=None, label: Optional[int]=None) -> List[Dict]:
        r"""Take an InputExample, and fill the tgt_text with label words
        """
        if example is not None and label is  None:
            label = example.label
        elif example is not None and label is not None:
            assert example.label == label, "Labels are not consitent. "

        if not isinstance(self.label_words[label], list):
        ...
        if len(instance_label_word) == 1:
            instance_label_word = instance_label_word[0]

        if example is None:
            tgt_text = instance_label_word
            return tgt_text
        else:
